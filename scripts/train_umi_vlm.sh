#!/usr/bin/env bash
# =============================================================================
# Hy-VLA UMI pretraining: 8-node x 8-GPU recipe (64 GPUs total).
#
# Loads ONLY the upstream Hy-Embodied VLM backbone into the inner
# ``dual_tower.vlm`` slot; the action expert is randomly initialized.
# Uses the ``umi_lance`` dataset config (single-frame, relative_chunk_ee_RT).
#
...
#   EXP_ID            hy_vlm_bootstrap_umi_lance
...
EXP_ID=${EXP_ID:-"hy_vlm_bootstrap_umi_lance"}
...
# --------- 2. Banner ---------
echo "========================================================"
echo "Hy-VLA UMI Lance pretraining (VLM bootstrap)"
echo "EXP_ID         : ${EXP_ID}"
