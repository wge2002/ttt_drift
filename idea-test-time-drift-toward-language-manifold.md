...
- **step 1 —— (a) 手动 CFG / velocity blend**(step 0 既已证明 masked 分支是合法先验,(a) 现在可行且最轻:每个 Euler step 跑两遍 denoise(full + masked),`v = v_masked + w·(v_full − v_masked)`,`w<1` 即往语言先验拉;`w` 可随偏离自适应。**无需训练,仅 2× 前向**)。先离线验证 `w→0` 时动作确实收敛到 masked 先验,再接 sim 测 OOD 成功率随 `w` 的变化——这才是核心假设的真正检验。
